  ...
  return (
    <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="font-medium text-gray-900">{resume?.name || '홍길동'}</p>
            </div>
            <div>
              <p className="text-gray-500">이메일</p>
              <p className="font-medium text-gray-900">{resume?.email || 'example@email.com'}</p>
            </div>
            <div>
              <p className="text-gray-500">전화번호</p>
              <p className="font-medium text-gray-900">{resume?.phone || '010-1234-5678'}</p>
            </div>
            <div>
              <p className="text-gray-500">GitHub</p>
              <p className="font-medium text-gray-900">{resume?.github || 'github.com/username'}</p>
            </div>
          </div>
        </div>
      </section>
      <section className="mb-8">
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <div className="mb-4">
            <h3 className="text-xl font-medium text-gray-900">{resume?.school || '대학교'}</h3>
            <p className="text-gray-700">{resume?.major || '컴퓨터공학과'} | {resume?.schoolPeriod || '2018-2022'}</p>
          </div>
        </div>
      </section>
      <section className="mb-8">
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <div className="mb-4">
            <h3 className="text-xl font-medium text-gray-900">{resume?.company || '회사명'}</h3>
            <p className="text-gray-700">{resume?.position || '직책'} | {resume?.workPeriod || '2022-현재'}</p>
      ...
      <section className="mb-8">
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <h3 className="font-medium mb-2 text-gray-900">프론트엔드</h3>
              <ul className="text-gray-700">
                {(resume?.frontend || 'React, Next.js, TypeScript').split(',').map((item: string, i: number) => <li key={i}>{item.trim()}</li>)}
              </ul>
            </div>
            <div>
              <h3 className="font-medium mb-2 text-gray-900">백엔드</h3>
              <ul className="text-gray-700">
